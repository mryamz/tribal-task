...
function typeHash(primaryType, types = {}) {
  const result = encodeType(primaryType, types);
...
function encodeData(primaryType, data, types = {}) {
  ...
  // Add field contents
  for (let field of types[primaryType]) {
    let value = data[field.name]; // maps to null sometimes
    if (field.type == 'string' || field.type == 'bytes') {
      ...
      if(!value) { // in certain cases, data[field.name] is null, thus value is null
        console.error("CHECK INPUT TO EIP712, ENSURE PARAM NAMES MATCH STANDARD.");
      }
