...
const { UInt256Max,toEBN,toBN } = require('../Utils/Ethereum');
  ...
  Object.entries(expectedRates).forEach(async ([kind, info]) => {
    ...
    describe(kind, () => {
      ...
      it('isInterestRateModel', async () => {
        expect(await model.isInterestRateModel());
      });
      ...
      it(`calculates correct borrow value`, async () => {
        ...
        await Promise.all(rateInputs.map(async ([cash, borrows, reserves = 0]) => {
          ...
          const actual = toBN(await getBorrowRate(model, cash, borrows, reserves)).dividedBy('1000000000000000000');
          const expected = toBN(rateFn(cash, borrows, reserves));
          const delta = actual.minus(expected);
          expect(delta.isLessThan('10000000'));
        }));
      });
      ...
      if (kind == 'jump-rate') {
        ...
        it('handles overflowed cash + borrows', async () => {
          await expect(getBorrowRate(model, UInt256Max(), UInt256Max(), 0)).to.be.revertedWith("SafeMath: addition overflow");
        });

        it('handles failing to get exp of borrows / cash + borrows', async () => {
          await expect(getBorrowRate(model, 0, UInt256Max(), 0)).to.be.revertedWith("SafeMath: multiplication overflow");
        });
        ...
        it('handles overflow utilization rate times slope', async () => {
          ...
          await expect(getBorrowRate(badModel, 1, 1, 0)).to.be.revertedWith("SafeMath: multiplication overflow");
        });
        ...
        it('handles overflow utilization rate times slope + base', async () => {
          ...
          await expect(getBorrowRate(badModel, 0, 1, 0)).to.be.revertedWith("SafeMath: multiplication overflow");
        });
      }
    });
          ...
          describe(`for jump=${jump}, kink=${kink}, base=${base}, slope=${slope}`, () => {
            ...
            points.forEach(([util, expected]) => {
              it(`and util=${util}%`, async () => {
                ...
                const actual = new BigNumber(borrowRateResult.toString()).dividedBy('10000000000000000').multipliedBy(blocksPerYear);
                const delta = (new BigNumber(expected).minus(actual)).absoluteValue();
                expect(delta.toNumber()).lessThan(0.01);
              });
            });
          });
        });
