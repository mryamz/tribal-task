...
describe('GovernorBravo#propose/5', () => {
  ...
  before(async () => {
    ...
    gov = await Gov.deploy(ethers.constants.AddressZero, comp.address, root.address, 17280, 1, "100000000000000000000000");
    await gov.deployed();
  ...
  describe("if there exists a pending or active proposal from the same proposer, we must revert.", () => {
    it("reverts with pending", async () => {
      ...
      ).to.be.revertedWith("GovernorBravo::propose: one live proposal per proposer, found an already pending proposal");
    });
    ...
    it("reverts with active", async () => {
      ...
      ).to.be.revertedWith("GovernorBravo::propose: one live proposal per proposer, found an already active proposal");
    });
  });
  ...
  describe("simple initialization", () => {
    ...
    it("Executed and Canceled flags are initialized to false", async () => {
      expect(trivialProposal[7]).equals(0);
    ...
    it("emits log with id and description", async () => {
      ...
      (await (await tx).wait()).events.forEach((e) => {
        ...
        expect(e.args['startBlock']).equals(15 + startingBlock);
        expect(e.args['endBlock']).equals(17295 + startingBlock + '');
